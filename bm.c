...
void
asciiDump(unsigned char *s, int len)
{
  unsigned char c;
  for (int i=0; i<len; i++) {
    c=s[i];
...
*/
int
openUDPSocket(uint32_t myaddr, unsigned short portno, int broadcast)
{
  int sockfd;
  int optval = 1;
  ...
  if (broadcast) {
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_BROADCAST, (void *)&optval, 
		   sizeof(optval)) < 0)
